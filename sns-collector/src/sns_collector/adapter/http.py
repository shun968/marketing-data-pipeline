...
from ..domain.collect import SourceUnavailable
...
@contextmanager
def source_errors() -> Iterator[None]:
    """通信例外を domain の `SourceUnavailable` へ変換する。

    収集ユースケースは `requests` を import できない（ADR-0011の層規則）。
    収集元アダプタは、外へ出す失敗を必ずこの形にしてから返す。
    """
    try:
        yield
