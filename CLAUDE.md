...
### 開発セッションはdevcontainer内で動かす

Claude Codeは `.devcontainer/` のコンテナ内で使う（ADR-0007）。ここが唯一のOS層の隔離境界であり、ホスト直起動のセッションには秘匿ファイルへの子プロセス経由のアクセスと外部通信を止める層が無い。層の全体像は `docs/isolation.md`。
